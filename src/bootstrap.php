...
function safe_return_path(mixed $value,string $fallback='/'):string{$value=trim((string)$value);$decoded=rawurldecode($value);if($value===''||!str_starts_with($value,'/')||str_starts_with($decoded,'//')||str_contains($decoded,'\\')||preg_match('/[\r\n]/',$value))return $fallback;$parts=parse_url($value);if($parts===false||isset($parts['scheme'])||isset($parts['host']))return $fallback;$path=(string)($parts['path']??'/');if(in_array($path,['/login','/logout','/forgot-password'],true))return $fallback;return $value;}
function verify_login_csrf():void{$provided=(string)($_POST['_csrf']??'');$current=(string)($_SESSION['csrf']??'');if($provided!==''&&$current!==''&&hash_equals($current,$provided))return;$expectedHost=strtolower(preg_replace('/:\d+$/','',(string)($_SERVER['HTTP_HOST']??'')));$origin=(string)($_SERVER['HTTP_ORIGIN']??$_SERVER['HTTP_REFERER']??'');$originHost=strtolower((string)parse_url($origin,PHP_URL_HOST));$sameOrigin=$expectedHost!==''&&$originHost!==''&&hash_equals($expectedHost,$originHost);$sameSite=strtolower((string)($_SERVER['HTTP_SEC_FETCH_SITE']??''))==='same-origin';if(!$sameOrigin&&!$sameSite){http_response_code(419);exit('Your session expired. Please return to the sign-in page and try again.');}$_SESSION['csrf']=bin2hex(random_bytes(32));}
...
function require_login(): void {if(user())return;$method=(string)($_SERVER['REQUEST_METHOD']??'GET');$target=$method==='GET'?(string)($_SERVER['REQUEST_URI']??'/'):(string)parse_url((string)($_SERVER['HTTP_REFERER']??'/'),PHP_URL_PATH);$target=safe_return_path($target);redirect('/login?return='.rawurlencode($target));}
...
function security_identity_hash(string $value): string {return hash_hmac('sha256',mb_strtolower(trim($value)),(string)config('key'));}
function rate_limit_hit(string $bucket,string $identity,int $limit,int $windowSeconds): bool {
    $bucket=mb_substr(preg_replace('/[^a-z0-9_.:-]+/i','_',strtolower($bucket)),0,100);$hash=security_identity_hash($identity);$cutoff=date('Y-m-d H:i:s',time()-max(1,$windowSeconds));
    try{$delete=db()->prepare('DELETE FROM request_rate_limits WHERE created_at<?');$delete->execute([date('Y-m-d H:i:s',time()-172800)]);$count=db()->prepare('SELECT COUNT(*) FROM request_rate_limits WHERE bucket=? AND identity_hash=? AND created_at>=?');$count->execute([$bucket,$hash,$cutoff]);if((int)$count->fetchColumn()>=$limit)return false;db()->prepare('INSERT INTO request_rate_limits(bucket,identity_hash) VALUES(?,?)')->execute([$bucket,$hash]);return true;}catch(Throwable $e){app_log('error','Rate limiter unavailable',['bucket'=>$bucket,'error'=>$e->getMessage()]);return false;}
}
function rate_limit_clear(string $bucket,string $identity): void {try{db()->prepare('DELETE FROM request_rate_limits WHERE bucket=? AND identity_hash=?')->execute([$bucket,security_identity_hash($identity)]);}catch(Throwable $e){}}
function enforce_rate_limit(string $bucket,string $identity,int $limit,int $windowSeconds,string $message='Too many requests. Please try again later.'): void {if(rate_limit_hit($bucket,$identity,$limit,$windowSeconds))return;http_response_code(429);header('Retry-After: '.max(60,$windowSeconds));exit($message);}
function sanitize_site_url(string $value): string {$value=trim(html_entity_decode($value,ENT_QUOTES|ENT_HTML5,'UTF-8'));if($value===''||str_starts_with($value,'#')||str_starts_with($value,'/'))return $value;$scheme=strtolower((string)parse_url($value,PHP_URL_SCHEME));return in_array($scheme,['http','https','mailto','tel'],true)?$value:'';}
function sanitize_site_html(string $html): string {
    if(trim($html)==='')return '';$doc=new DOMDocument('1.0','UTF-8');$previous=libxml_use_internal_errors(true);$doc->loadHTML('<?xml encoding="utf-8" ?><div id="opencrm-site-root">'.$html.'</div>',LIBXML_HTML_NOIMPLIED|LIBXML_HTML_NODEFDTD);libxml_clear_errors();libxml_use_internal_errors($previous);$root=$doc->getElementById('opencrm-site-root');if(!$root)return '';
    $blocked=['script','object','embed','applet','base','meta','link','style','svg','math'];$walker=function(DOMNode $node)use(&$walker,$blocked):void{foreach(iterator_to_array($node->childNodes) as $child){if($child instanceof DOMElement){if(in_array(strtolower($child->tagName),$blocked,true)){$child->parentNode?->removeChild($child);continue;}foreach(iterator_to_array($child->attributes) as $attr){$name=strtolower($attr->name);$value=$attr->value;if(str_starts_with($name,'on')||in_array($name,['srcdoc','formaction','integrity','nonce'],true)){$child->removeAttributeNode($attr);continue;}if(in_array($name,['href','src','action','poster','background','xlink:href'],true)){$safe=sanitize_site_url($value);if($safe==='')$child->removeAttributeNode($attr);else$child->setAttribute($attr->name,$safe);}}$walker($child);}}};$walker($root);$out='';foreach(iterator_to_array($root->childNodes) as $child)$out.=$doc->saveHTML($child);return $out;
}
...
function audit_record_label(string $entity,?int $entityId):?string{if(!$entityId)return null;$sources=['contact'=>['contacts',"TRIM(CONCAT(first_name,' ',last_name))"],'company'=>['companies','name'],'opportunity'=>['opportunities','title'],'event'=>['events','title'],'reminder'=>['reminders',"CONCAT(message,' — due ',due_at)"],'event_reminder'=>['event_reminders',"CONCAT(message,' — due ',due_at)"],'system_alert'=>['system_alerts',"CONCAT(message,' — due ',due_at)"],'booking'=>['bookings',"CONCAT(attendee_name,' — ',starts_at)"],'tag'=>['tags','name'],'user'=>['users',"COALESCE(NULLIF(display_name,''),username)"],'resource'=>['resources','title'],'resource_portal'=>['resource_portals','name'],'site'=>['sites','name'],'site_page'=>['site_pages','name'],'crm_form'=>['crm_forms','name'],'lead_magnet'=>['lead_magnets','title'],'email_mailbox'=>['email_mailboxes','name'],'workflow'=>['workflows','name'],'quote'=>['quotes','title']];if(!isset($sources[$entity]))return null;[$table,$expression]=$sources[$entity];try{$q=db()->prepare("SELECT $expression FROM $table WHERE id=?");$q->execute([$entityId]);$label=trim((string)$q->fetchColumn());return $label!==''?$label:null;}catch(Throwable $e){return null;}}
...
function app_settings(bool $refresh=false): array {
    ...
    if ($settings === null) {
        $settings = ['app_name'=>'OpenCRM','primary_color'=>'#1565c0','accent_color'=>'#43a047','logo_path'=>'','logo_mime'=>'','logging_level'=>'info','app_timezone'=>'America/New_York','mail_transport'=>'local','mail_from_address'=>'','mail_from_name'=>'','smtp_host'=>'','smtp_port'=>'587','smtp_encryption'=>'tls','smtp_username'=>'','smtp_password_enc'=>'','openai_api_key_enc'=>'','openai_model'=>'gpt-5.4-mini','booking_engine'=>'native','easyappointments_url'=>'','easyappointments_api_token_enc'=>'','google_calendar_client_id'=>'','google_calendar_client_secret_enc'=>'','microsoft_calendar_client_id'=>'','microsoft_calendar_client_secret_enc'=>'','microsoft_calendar_tenant'=>'common','stripe_secret_key_enc'=>'','stripe_webhook_secret_enc'=>'','stripe_enabled'=>'0','operational_alerts_enabled'=>'1','operational_alert_email'=>'','operational_alert_level'=>'error','operational_alert_cooldown_hours'=>'6','operational_webhook_failure_threshold'=>'5','legal_company_name'=>'','legal_contact_email'=>'','legal_company_address'=>'','legal_privacy_policy'=>'','legal_terms_of_use'=>'','legal_cookie_notice'=>'We use privacy-conscious analytics to understand visits and improve this site.','legal_marketing_consent'=>'I agree to receive relevant email communications. I can unsubscribe at any time.','legal_retention_policy'=>'CRM records are retained while needed for legitimate business purposes and legal obligations, then securely deleted or anonymized.','legal_form_disclosure'=>'By submitting this form, you acknowledge the Privacy Policy and Terms of Use.','legal_require_form_consent'=>'0'];
        try {
...
function crm_select_mailbox(?int $mailboxId=null):?array{try{$where="m.active=1 AND m.health_status<>'disabled'";$args=[];if($mailboxId){$where.=' AND m.id=?';$args[]=$mailboxId;}$sql="SELECT m.*,(SELECT COUNT(*) FROM email_mailbox_sends h WHERE h.mailbox_id=m.id AND h.status='sent' AND h.sent_at>=DATE_SUB(NOW(),INTERVAL 1 HOUR)) sent_hour,(SELECT COUNT(*) FROM email_mailbox_sends d WHERE d.mailbox_id=m.id AND d.status='sent' AND d.sent_at>=CURRENT_DATE) sent_day FROM email_mailboxes m WHERE $where HAVING sent_hour<hourly_limit AND sent_day<daily_limit ORDER BY (sent_day/NULLIF(daily_limit,0))/GREATEST(weight,1),last_used_at IS NOT NULL,last_used_at,id LIMIT 1";$q=db()->prepare($sql);$q->execute($args);return $q->fetch()?:null;}catch(Throwable $e){return null;}}
function crm_queue_email(string $to,string $subject,string $body,bool $transactional=false,?int $mailboxId=null):int{$q=db()->prepare('INSERT INTO email_queue(mailbox_id,recipient,subject,body_text,transactional,created_by) VALUES(?,?,?,?,?,?)');$q->execute([$mailboxId,$to,$subject,$body,$transactional?1:0,user()['id']??null]);return (int)db()->lastInsertId();}
function crm_send_email(string $to,string $subject,string $body,bool $transactional=true,?int $mailboxId=null): bool {
    app_settings(true);
    ...
    if(!$transactional&&app_setting('legal_company_address')!=='')$body=rtrim($body)."\n\n".(app_setting('legal_company_name')?:app_setting('app_name','OpenCRM'))."\n".app_setting('legal_company_address');
    if(!filter_var($to,FILTER_VALIDATE_EMAIL))throw new InvalidArgumentException('Recipient email is invalid.');$mailbox=crm_select_mailbox($mailboxId);$from=$mailbox['from_address']??app_setting('mail_from_address','notifications@'.(parse_url((string)config('url'),PHP_URL_HOST)?:'localhost'));$fromName=$mailbox['from_name']??app_setting('mail_from_name',app_setting('app_name','OpenCRM'));$headers=['From: '.$fromName.' <'.$from.'>','MIME-Version: 1.0','Content-Type: text/plain; charset=UTF-8','X-Auto-Response-Suppress: All'];if(!empty($mailbox['reply_to']))$headers[]='Reply-To: '.$mailbox['reply_to'];
    if(!$mailbox&&app_setting('mail_transport','local')==='local')return @mail($to,$subject,$body,implode("\r\n",$headers));
    $host=$mailbox['smtp_host']??app_setting('smtp_host');$port=(int)($mailbox['smtp_port']??app_setting('smtp_port','587'));$encryption=$mailbox['smtp_encryption']??app_setting('smtp_encryption','tls');if($host===''||!filter_var($from,FILTER_VALIDATE_EMAIL))throw new RuntimeException('SMTP host and sender address are required.');$target=($encryption==='ssl'?'ssl://':'').$host.':'.$port;$socket=@stream_socket_client($target,$errno,$error,15,STREAM_CLIENT_CONNECT);if(!$socket)throw new RuntimeException("SMTP connection failed: $error ($errno)");stream_set_timeout($socket,15);try{$hello=smtp_read($socket);if((int)substr($hello,0,3)!==220)throw new RuntimeException('SMTP greeting failed.');smtp_command($socket,'EHLO '.(parse_url((string)config('url'),PHP_URL_HOST)?:'localhost'),[250]);if($encryption==='tls'){smtp_command($socket,'STARTTLS',[220]);if(!stream_socket_enable_crypto($socket,true,STREAM_CRYPTO_METHOD_TLS_CLIENT))throw new RuntimeException('SMTP TLS negotiation failed.');smtp_command($socket,'EHLO '.(parse_url((string)config('url'),PHP_URL_HOST)?:'localhost'),[250]);}$username=$mailbox['smtp_username']??app_setting('smtp_username');$password=decrypt_secret($mailbox['smtp_password_enc']??app_setting('smtp_password_enc'));if($username!==''){smtp_command($socket,'AUTH LOGIN',[334]);smtp_command($socket,base64_encode($username),[334]);smtp_command($socket,base64_encode($password),[235]);}smtp_command($socket,'MAIL FROM:<'.$from.'>',[250]);smtp_command($socket,'RCPT TO:<'.$to.'>',[250,251]);smtp_command($socket,'DATA',[354]);$message='Subject: '.$subject."\r\n".'To: <'.$to.'>'."\r\n".implode("\r\n",$headers)."\r\n\r\n".$body;$message=preg_replace('/(?m)^\./','..',str_replace(["\r\n","\r"],"\n",$message));fwrite($socket,str_replace("\n","\r\n",$message)."\r\n.\r\n");$response=smtp_read($socket);if((int)substr($response,0,3)!==250)throw new RuntimeException('SMTP message delivery was rejected: '.trim($response));smtp_command($socket,'QUIT',[221]);if($mailbox){db()->prepare("INSERT INTO email_mailbox_sends(mailbox_id,recipient,message_type,status) VALUES(?,?,?,'sent')")->execute([$mailbox['id'],strtolower($to),$transactional?'transactional':'marketing']);db()->prepare("UPDATE email_mailboxes SET last_used_at=NOW(),health_status='healthy',last_error=NULL WHERE id=?")->execute([$mailbox['id']]);}return true;}catch(Throwable $e){if($mailbox){db()->prepare("INSERT INTO email_mailbox_sends(mailbox_id,recipient,message_type,status,error_message) VALUES(?,?,?,'failed',?)")->execute([$mailbox['id'],strtolower($to),$transactional?'transactional':'marketing',mb_strimwidth($e->getMessage(),0,1000)]);db()->prepare("UPDATE email_mailboxes SET health_status='warning',last_error=? WHERE id=?")->execute([mb_strimwidth($e->getMessage(),0,1000),$mailbox['id']]);}throw $e;}finally{fclose($socket);}
}
function operational_notify(string $key,string $status,string $message,array $details=[]):bool{
    $status=in_array($status,['healthy','warning','error'],true)?$status:'error';$now=(new DateTimeImmutable())->format('Y-m-d H:i:s');
    try{$q=db()->prepare('SELECT * FROM operational_alert_states WHERE alert_key=?');$q->execute([$key]);$previous=$q->fetch()?:null;$failures=$status==='healthy'?0:(int)($previous['consecutive_failures']??0)+1;$notify=false;$recovery=$status==='healthy'&&$previous&&$previous['current_status']!=='healthy';$threshold=app_setting('operational_alert_level','error');$rank=['healthy'=>0,'warning'=>1,'error'=>2];$eligible=($rank[$status]??2)>=($rank[$threshold]??2);$cooldown=max(1,min(168,(int)app_setting('operational_alert_cooldown_hours','6')));if($recovery)$notify=true;elseif($eligible&&(!$previous||$previous['current_status']!==$status||!$previous['last_notified_at']||strtotime($previous['last_notified_at'])<time()-$cooldown*3600))$notify=true;
        db()->prepare('INSERT INTO operational_alert_states(alert_key,current_status,consecutive_failures,last_message,last_notified_at,last_checked_at,recovered_at) VALUES(?,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE current_status=VALUES(current_status),consecutive_failures=VALUES(consecutive_failures),last_message=VALUES(last_message),last_notified_at=IF(VALUES(last_notified_at) IS NULL,last_notified_at,VALUES(last_notified_at)),last_checked_at=VALUES(last_checked_at),recovered_at=VALUES(recovered_at)')->execute([$key,$status,$failures,mb_substr($message,0,1000),$notify?$now:null,$now,$recovery?$now:null]);
        $to=trim(app_setting('operational_alert_email'));if(!$notify||app_setting('operational_alerts_enabled','1')!=='1'||!filter_var($to,FILTER_VALIDATE_EMAIL))return false;$label=$recovery?'RECOVERED':strtoupper($status);$body=$message."\n\nIncident: $key\nStatus: $label\nTime: $now ".app_setting('app_timezone','America/New_York').($details?"\n\nDetails:\n".json_encode($details,JSON_PRETTY_PRINT|JSON_UNESCAPED_SLASHES):'')."\n\nSystem health: ".rtrim((string)config('url'),'/').'/admin/health';try{crm_send_email($to,'['.$label.'] '.app_setting('app_name','OpenCRM').' operational alert',$body,true);}catch(Throwable $mailError){db()->prepare('UPDATE operational_alert_states SET last_notified_at=NULL WHERE alert_key=?')->execute([$key]);throw $mailError;}return true;
    }catch(Throwable $e){error_log('Operational notification failed: '.$e->getMessage());return false;}
}
