...
if(preg_match('#^/alerts/(contact|event)/(\d+)$#',$path,$m)&&$method==='POST'){
    ...
    if(post('action')==='complete'){
        if($table==='reminders')db()->prepare("UPDATE reminders SET due_at=DATE_ADD(GREATEST(due_at,NOW()),INTERVAL recurrence_days DAY),completed_at=NULL WHERE id=? AND user_id=? AND recurrence_days>0")->execute([$id,user()['id']]);
        $advanced=$table==='reminders'&&db()->query('SELECT ROW_COUNT()')->fetchColumn();
        if(!$advanced)db()->prepare("UPDATE $table SET completed_at=NOW() WHERE id=? AND user_id=?")->execute([$id,user()['id']]);
    }
    ...
    audit(post('action')==='complete'&&($advanced??false)?'advance_recurring':(post('action')==='complete'?'complete':'snooze'),$m[1].'_reminder',$id);redirect('/alerts');
}
