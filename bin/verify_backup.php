...
$markers=['CREATE TABLE `contacts`'=>false,'CREATE TABLE `users`'=>false,'CREATE TABLE `opportunities`'=>false];
$gz=gzopen($dbFile,'rb');if($gz===false)throw new RuntimeException('Database dump could not be opened.');$tail='';
while(!gzeof($gz)){
    $chunk=gzread($gz,131072);if($chunk===false){gzclose($gz);throw new RuntimeException('Database dump could not be read.');}
    $scan=$tail.$chunk;foreach($markers as $marker=>$found)if(!$found&&str_contains($scan,$marker))$markers[$marker]=true;$tail=substr($scan,-64);
}
gzclose($gz);
foreach($markers as $marker=>$found)if(!$found)throw new RuntimeException('Database dump is missing '.$marker);
