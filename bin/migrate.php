...
if(!in_array('structured_data_json',$pageColumns,true))db()->exec('ALTER TABLE site_pages ADD structured_data_json JSON NULL AFTER noindex');
$siteColumns=db()->query("SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='sites'")->fetchAll(PDO::FETCH_COLUMN);
if(!in_array('robots_extra',$siteColumns,true))db()->exec('ALTER TABLE sites ADD robots_extra TEXT NULL AFTER retention_days');
if(!in_array('search_console_verification',$siteColumns,true))db()->exec('ALTER TABLE sites ADD search_console_verification VARCHAR(255) NULL AFTER robots_extra');
...
$roleRows=db()->query('SELECT id,permissions_json FROM roles')->fetchAll();$saveRole=db()->prepare('UPDATE roles SET permissions_json=? WHERE id=?');foreach($roleRows as $roleRow){$rp=json_decode((string)$roleRow['permissions_json'],true)?:[];$add=['alerts.view','alerts.edit'];if(in_array('contacts.view',$rp,true)||in_array('contacts.edit',$rp,true))$add=array_merge($add,['companies.view','opportunities.view']);if(in_array('contacts.edit',$rp,true))$add=array_merge($add,['companies.edit','opportunities.edit']);$partnerCheck=db()->prepare('SELECT COUNT(*) FROM users WHERE role_id=? AND partner_sales_access=1');$partnerCheck->execute([$roleRow['id']]);if($partnerCheck->fetchColumn())$add[]='partner_sales.view';$saveRole->execute([json_encode(array_values(array_unique(array_merge($rp,$add)))),$roleRow['id']]);}
$permissions = json_encode(['contacts.view','contacts.edit','companies.view','companies.edit','events.view','events.edit','opportunities.view','opportunities.edit','alerts.view','alerts.edit','partner_sales.view','partner_sales.edit','reports.view','reports.edit','lead_magnets.view','lead_magnets.edit','forms.view','forms.edit','promotional_links.view','promotional_links.edit','sites.view','sites.edit','bookings.view','bookings.edit','communications.view','communications.edit','workflows.view','workflows.edit','resources.view','resources.edit','sales_documents.view','sales_documents.edit']);
